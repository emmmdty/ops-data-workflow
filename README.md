...
- `config/channel_profiles.yml`：渠道配置，默认覆盖小红书商业化、小红书市场部、抖音市场部、抖音商业化、B站、微信市场部、微信商业化。
...
系统会按 `config/channel_profiles.yml` 识别渠道。新平台优先新增配置，不改代码。
...
每个渠道 sheet 先展示统一字段，再展示原始剩余字段。未知原始字段会保留为 `原始字段__字段名`，便于回查。
