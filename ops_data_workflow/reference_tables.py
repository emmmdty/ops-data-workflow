...
DEFAULT_PROCESSING_RULES = pd.DataFrame(
    [
        {"规则": "去重键", "取值": "渠道 + 视频/笔记id", "说明": "小红书/B站只按同渠道视频/笔记id合并；ID为空不按标题兜底。"},
        {"规则": "数值冲突阈值", "取值": "0.05", "说明": "小红书/B站重复行基础指标直接求和；其他渠道相对差异大于5%时求和。"},
        {"规则": "一级分类", "取值": "不启用", "说明": "不区分长视频、短视频、图文；渠道就是第一层分析维度。"},
        {"规则": "B站栏目题材", "取值": "采访 / 新手教学", "说明": "B站不做动态栏目题材分类，统一按固定值写入。"},
        {"规则": "小红书/抖音TAG", "取值": "TAG权威映射", "说明": "TAG命中时作为二级栏目来源，冲突进入质量诊断。"},
    ]
)
